...
- 當本地存儲已滿或不可用時，核心功能（即時價格、K線圖表）繼續運作，自選清單功能禁用並顯示警告「無法保存收藏，頁面刷新後清單將丟失」
- 若同時訂閱超過 100 種幣種，如何確保 UI 響應性能不受影響？
...
- **SC-004**: 應用應能同時處理至少 100 種幣種的實時更新（最大訂閱上限），UI 帧率保持在 60 FPS 以上
...
- Q: 選擇加密貨幣即時資料源 → A: Binance WebSocket（公開層級）- 工業級穩定性，廣泛的幣種覆蓋，支援多種推送格式，無認證需求的免費層級
- Q: WebSocket 推送數據格式與更新頻率 → A: 實時推送（tick）+ 聚合交易流（aggTrade）- 高頻率更新，結合價格滴答與聚合交易數據，實現最小延遲
- Q: 訂閱幣種延展性邊界 → A: 100 種幣種 - 平衡延展性與性能，展示學習成效同時保持代碼理解性
- Q: 本地存儲降級與失敗處理策略 → A: 部分功能降級 + 使用者警告 - 核心功能繼續，自選清單臨時存在記憶體中，刷新即失失
- Q: K 線圖表庫選擇 → A: TradingView Lightweight Charts - 輕量、高效、金融專用、API 簡潔，最適合學習

## Assumptions

- 使用 Binance WebSocket 公開層級作為即時行情數據源，推送格式採用 tick（實時價格推送）+ aggTrade（聚合交易流），實現 <1 秒延遲
- 最大訂閱上限為 100 種幣種，確保 60 FPS UI 性能
- 使用 React 框架搭建，搭配 TypeScript 確保型別安全
- 使用 TanStack Query（React Query）進行快取與資料同步管理
- 使用 TradingView Lightweight Charts 渲染 K 線圖表，提供高效的金融圖表體驗
- 使用瀏覽器原生 localStorage 作為本地存儲解決方案；localStorage 不可用時，核心功能繼續運作，自選清單功能禁用並顯示警告
